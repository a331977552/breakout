...
public class TileMap_2942625
{
	...
	public int getPixelHeight() {
		return mapHeight * tileHeight + (mapHeight+1 )* TILE_GAP;
	}
	...
	public int getPixelWidth() {
		return mapWidth * tileWidth + (mapWidth+1 )* TILE_GAP;
	}
		...
		try
		{
			...
			while ((line = in.readLine()) != null)
			{
				...
				if (trimmed.charAt(0) == '#') // Look for a character to image map
				{
					...
					Image img  = new ImageIcon(  "images/" + fileName).getImage().getScaledInstance(tileWidth, tileHeight, Image.SCALE_DEFAULT);
	...
	public void draw(Graphics2D g)
	{
		...
		for (int r=0; r<mapHeight; r++)
		{
			for (int c=0; c<mapWidth; c++)
			{
				...
				xc =  c*tileWidth + (c+1) * TILE_GAP;
				yc =  r*tileHeight + (r+1) * TILE_GAP;
				if(getTile(c,r).isDestroy())continue;
	...
	public void resetTiles() {
		this.tiles.stream().forEach(tile -> {
			tile.setDestroy(false);
		});
	}
}
